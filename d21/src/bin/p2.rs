...
fn shortest_path_length_depth_0(
    ...
    let length = [high_road, low_road]
        ...
        .map(|road| {
            ...
            road.insert(0, b'A');
            road.windows(2)
                .map(|w| shortest_path_length_depth_n(w[0], w[1], 1, memo))
                .sum::<u64>()
        })
        .min()
        .unwrap();
...
fn shortest_path_length_depth_n(
    start: u8,
    end: u8,
    depth: Depth,
    memo: &mut HashMap<(u8, u8, Depth), u64>,
    ...
    if depth == 25 {
        return shortest_path_length_depth_25(start, end);
    }
    if let Some(length) = memo.get(&(start, end, depth)) {
        return *length;
    }
    let map_to_coordinates = |ch: u8| -> (usize, usize) {
        match ch {
    ...
    let length = [high_road, low_road]
        ...
        .map(|road| {
            ...
            road.insert(0, b'A');
            road.windows(2)
                .map(|w| shortest_path_length_depth_n(w[0], w[1], depth + 1, memo))
                .sum::<u64>()
        })
        .min()
        .unwrap();
    memo.insert((start, end, depth), length);
    length
}

fn shortest_path_length_depth_25(start: u8, end: u8) -> u64 {
    let map_to_coordinates = |ch: u8| -> (usize, usize) {
        match ch {
            b'^' => (0, 1),
            b'A' => (0, 2),
